mod wmww_json {
    ...
    fn parse_json_array(mut iter: Chars) -> (Vec<JsonValue>, Chars) {
    ...
    fn parse_json_object(mut iter: Chars) -> (HashMap<String, JsonValue>, Chars) {
        let (start_token, mut iter) = next_token(iter);
    ...
    fn parse_json_value(mut iter: Chars) -> (JsonValue, Chars) {
    ...
    pub fn parse_json(mut iter: Chars) -> (JsonValue, Chars) {
        parse_json_value(iter)
    }
}
